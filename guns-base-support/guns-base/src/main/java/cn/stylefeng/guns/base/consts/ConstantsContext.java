...
public class ConstantsContext {
    ...
    public static String getSystemName() {
        ...
        if (ToolUtil.isEmpty(systemName)) {
            log.error("系统常量存在空值！常量名称：GUNS_SYSTEM_NAME，采用默认名称：E365管理平台" + TIPS_END);
            return "E365管理平台";
        } else {
            return systemName;
        }
    ...
    public static String getDefaultPassword() {
        ...
        if (ToolUtil.isEmpty(defaultPassword)) {
            log.error("系统常量存在空值！常量名称：GUNS_DEFAULT_PASSWORD，采用默认密码：123456" + TIPS_END);
            return "123456";
        } else {
    ...
    public static int getScore() {
        String tenantOpen = (String) CONSTNTS_HOLDER.get("score");
        if (ToolUtil.isEmpty(tenantOpen)) {
            log.error("系统常量存在空值！常量名称：GUNS_TENANT_OPEN，采用默认值：DISABLE" + TIPS_END);
            return 6;
        } else {
            return Integer.parseInt(tenantOpen);
        }
    }
