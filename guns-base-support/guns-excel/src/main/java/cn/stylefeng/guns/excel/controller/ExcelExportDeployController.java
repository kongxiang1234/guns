...
public class ExcelExportDeployController extends BaseController {
    ...
    public void download(@RequestParam("fileFinalName") String fileFinalName, HttpServletResponse httpServletResponse) {

        //上传路径设置
        String fileSavePath = ConstantsContext.getFileUploadPath();
        fileSavePath = fileSavePath + EXCEL_FILE_TEMPLATE_PATH;
        String name = fileFinalName.substring(fileFinalName.lastIndexOf("/")+1);
        //查找文件信息
        FileInfo fileInfo = fileInfoService.getByFinalName(name);

        try {
            FileDownload.fileDownload(httpServletResponse, fileSavePath + name, fileInfo.getFileName());
        } catch (Exception e) {
            e.printStackTrace();
        }
