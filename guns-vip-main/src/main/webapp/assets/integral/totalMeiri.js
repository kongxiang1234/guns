...
layui.use(['table', 'admin', 'ax', 'func','form','laydate'], function () {
    ...
    Integral.initColumn = function () {
        return [[
            ...
            {field: 'qtotal', sort: true, title: '缺练习次数', templet: function (d) {
                    var etime=$("#etime").val();
                    var btime=$("#btime").val();
                    var url = Feng.ctxPath + '/practice/queTotalMeiri?account=' + d.account+"&btime="+btime+"&etime="+etime;
                    return '<a style="color: #01AAED;" href="' + url + '">' + d.qtotal + '</a>';
                }}
        ]];
    };
